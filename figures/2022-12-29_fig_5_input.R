...
### December 29, 2022 
### rAF/Indels Projet
### This code will be to create figure 5 csv input 
library(tidyverse)
library(data.table)
library(readxl)
library(dplyr)
...
IGM_sAF_rAF = fread("2022-12-30_IGM_sAF_rAF.csv", sep = ",", header = TRUE)
gnomad_sAF_rAF = fread("2022-12-30_gnomAD_sAF_rAF.csv", sep = ",", header = TRUE)
...
#test
#gnomad_lcr_in_clinvar = merge(gnomad_lcr, clinvar, by = "VarID")
#gnomad_B_LB_lcr = gnomad_lcr_in_clinvar %>% filter(V10 %in% benign | V10 %in% LB) %>% filter (sAF <= rare)
#gnomad_P_LP_lcr = gnomad_lcr_in_clinvar %>% filter(V10 %in% patho | V10 %in% LP) %>% filter (sAF <= rare)

#test
#IGM_lcr_in_clinvar = merge(IGM_lcr, clinvar, by = "VarID")
#IGM_B_LB_lcr = IGM_lcr_in_clinvar %>% filter(V10 %in% benign | V10 %in% LB) %>% filter (sAF <= rare)
#IGM_P_LP_lcr = IGM_lcr_in_clinvar %>% filter(V10 %in% patho | V10 %in% LP) %>% filter (sAF <= rare)
...
gnomad_B_LB$log_key = 0 #1840
gnomad_P_LP$log_key = 1 #7418

#IGM
IGM_B_LB$log_key = 0 #286
IGM_P_LP$log_key = 1 #4053
...
fwrite(gnomad_output, paste0( Sys.Date(), "_figure5_gnomad_input.csv"))
fwrite(IGM_output, paste0( Sys.Date(), "_figure5_IGM_input.csv"))
